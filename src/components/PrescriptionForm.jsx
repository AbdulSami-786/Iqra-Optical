...
const PrescriptionForm = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setRightEye({ ...emptyEye, ...initialData.rightEye });
      setLeftEye({ ...emptyEye, ...initialData.leftEye });
    }
  }, [initialData]);
  ...
  const renderEyeFields = (eye, values) => (
    <div className="grid grid-cols-3 gap-2 sm:gap-4">
      <div>
        ...
        <input
          ...
          step="0.25"
          value={values.sphere}
          onChange={(e) => handleEyeChange(eye, 'sphere', e.target.value)}
          placeholder="0.00"
          className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-xl focus:border-ink outline-none text-sm"
        />
      </div>
      <div>
        <label className="text-xs text-gray-500 block mb-1">Cylinder</label>
        <input
          type="number"
          step="0.25"
          value={values.cylinder}
          onChange={(e) => handleEyeChange(eye, 'cylinder', e.target.value)}
          placeholder="0.00"
          className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-xl focus:border-ink outline-none text-sm"
        />
      </div>
      <div>
        ...
        <input
          ...
          max="180"
          value={values.axis}
          onChange={(e) => handleEyeChange(eye, 'axis', e.target.value)}
          placeholder="0"
          className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-xl focus:border-ink outline-none text-sm"
        />
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm">
          <motion.div
            ...
            className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-2xl w-full max-h-[92vh] sm:max-h-[90vh] flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 shrink-0">
              <h2 className="text-lg sm:text-xl font-serif font-semibold text-ink">Prescription Details</h2>
              <button
                onClick={onClose}
            ...
            <div className="p-4 sm:p-6 overflow-y-auto">
              ...
              <div className="mb-6">
                ...
                {renderEyeFields('right', rightEye)}
                <p className="text-[10px] text-gray-400 mt-1">(step 0.25 to 0.5) • axis step 1°</p>
              </div>

              {/* Left Eye */}
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-ink mb-2">Left Eye</h4>
                {renderEyeFields('left', leftEye)}
                <p className="text-[10px] text-gray-400 mt-1">(step 0.25 to 0.5) • axis step 1°</p>
              </div>
            ...
            {/* Actions */}
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 p-4 sm:p-6 pt-3 sm:pt-4 border-t border-gray-100 shrink-0">
              <button
                onClick={onClose}
